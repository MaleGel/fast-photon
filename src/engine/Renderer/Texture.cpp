...
// ── Private shared upload path ───────────────────────────────────────────────

bool Texture::uploadRgba(const VulkanContext& ctx, const uint8_t* pixels,
                         uint32_t width, uint32_t height, bool srgb) {
    const VkFormat fmt = srgb ? VK_FORMAT_R8G8B8A8_SRGB
                              : VK_FORMAT_R8G8B8A8_UNORM;

    FP_CORE_ASSERT(pixels != nullptr && width > 0 && height > 0,
                   "Texture::uploadRgba called with empty input");
    ...
    if (vmaCreateImage(ctx.allocator(), &imageCI, &imageAllocCI,
        ...
        FP_CORE_ERROR("vmaCreateImage failed ({}x{})", width, height);
        return false;
    }
...
bool Texture::loadFromFile(const VulkanContext& ctx, const std::string& path) {
    ...
    // PNG content is sRGB — hardware will convert to linear on sample.
    const bool ok = uploadRgba(ctx, pixels, static_cast<uint32_t>(w),
                               static_cast<uint32_t>(h), /*srgb*/ true);
    stbi_image_free(pixels);

    if (ok) {
        FP_CORE_INFO("Texture loaded: '{}' ({}x{}, {} bytes)",
                     path, m_width, m_height, m_width * m_height * 4u);
    }
    return ok;
}

bool Texture::loadFromMemoryRGBA(const VulkanContext& ctx, const uint8_t* pixels,
                                 uint32_t width, uint32_t height) {
    // In-memory upload path is used by generated data (font atlases, masks).
    // Keep those linear — they aren't perceptual colour.
    if (!uploadRgba(ctx, pixels, width, height, /*srgb*/ false)) return false;
    FP_CORE_TRACE("Texture uploaded from memory ({}x{}, {} bytes)",
                  width, height, width * height * 4u);
    return true;
}
