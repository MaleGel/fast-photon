...
void VulkanContext::pickPhysicalDevice() {
    ...
    // Resolve MSAA sample count. Framebuffer-level caps are the AND of color
    // and depth caps — both attachments must share the same sample count.
    const VkSampleCountFlags counts = props.limits.framebufferColorSampleCounts
                                    & props.limits.framebufferDepthSampleCounts;

    // Target 4×; fall back down on GPUs that don't support it.
    ...
    FP_CORE_INFO("MSAA: {}×", static_cast<int>(m_msaaSamples));
}
