...
void InputMap::init(EventBus& bus) {
    ...
    m_mouseDownSub = bus.subscribe<MouseButtonPressedEvent>(
        [this](const MouseButtonPressedEvent& e) { onMouseDown(e.button); });

    m_mouseUpSub   = bus.subscribe<MouseButtonReleasedEvent>(
        [this](const MouseButtonReleasedEvent& e) { onMouseUp(e.button); });
...
void InputMap::shutdown() {
    ...
    m_keyUpSub.release();
    m_mouseDownSub.release();
    m_mouseUpSub.release();
    m_wheelSub.release();
    clearBindings();
    m_bus = nullptr;
}
...
void InputMap::bindAction(ActionID action, Binding binding) {
    ...
    switch (binding.type) {
        ...
        case Binding::Type::MouseButton:
            m_mouseButtonToActions[binding.button].insert(action);
            break;
...
static Binding parseBinding(const std::string& token) {
    ...
    if (token == "Mouse1")         return Binding::mouseButton(SDL_BUTTON_LEFT);
    if (token == "Mouse2")         return Binding::mouseButton(SDL_BUTTON_RIGHT);
    if (token == "Mouse3")         return Binding::mouseButton(SDL_BUTTON_MIDDLE);
...
void InputMap::onMouseDown(uint8_t button) {
    auto it = m_mouseButtonToActions.find(button);
    if (it == m_mouseButtonToActions.end()) return;
...
void InputMap::onMouseUp(uint8_t button) {
    auto it = m_mouseButtonToActions.find(button);
    if (it == m_mouseButtonToActions.end()) return;

    // Same held-across-multiple-bindings logic as keys: an action stays down
    // while any of its mouse buttons is pressed.
    const uint32_t state = SDL_GetMouseState(nullptr, nullptr);
    for (ActionID action : it->second) {
        bool anyStillDown = false;
        for (const auto& binding : m_actionBindings[action]) {
            if (binding.type != Binding::Type::MouseButton) continue;
            if (state & SDL_BUTTON(binding.button)) { anyStillDown = true; break; }
        }
        if (!anyStillDown) {
            m_actionsDown.erase(action);
            m_bus->publish(ActionReleasedEvent{ action });
